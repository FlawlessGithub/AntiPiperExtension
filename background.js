...
function checkUrl(url) {
	for (let blockedUrl of urlList){
		...
		console.log(urlList);
		console.log(" ") // Dead space for improved console readability
}
chrome.history.search({
	...
	maxResults: 1
		...
		if (checkUrl(historyEntry[0].url) == true) {
			chrome.history.deleteUrl({"url": historyEntry[0].url});
			console.log(historyEntry[0].url + " and all identical URLs have been deleted from the browser history.");
		}
	});
